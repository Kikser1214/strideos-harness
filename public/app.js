...
function renderDashboard(dashboard) {
  ...
  $("#connectorNote").textContent = `${dashboard.connector?.label || "No provider route"}. No direct Garmin agent route is claimed; use an approved export or manual entry.`;
...
function renderLedger(decision, { restored = false } = {}) {
  ...
  const isStop = decision.status === "stopped"; const isAuto = decision.status === "completed"; const isVerification = decision.status === "verification_required"; const isResolved = ["approved", "declined"].includes(decision.status); const evidence = decision.evidence.map(escapeHtml).join(" · ");
  const gateLabel = isVerification ? "Write outcome needs reconciliation" : decision.gate.mode === "confirm" ? "Approval boundary reached" : decision.gate.mode === "stop" ? "Safety stop triggered" : "Autonomous action allowed";
  let actionTitle = isVerification ? "Verification required" : isStop ? "Action stopped" : isAuto ? "Action completed" : "Waiting for the athlete"; let actionDetail = isVerification ? decision.result?.message || "Check the provider before creating any retry." : decision.gate.action.replaceAll("_", " ");
  ...
  if (decision.status === "awaiting_approval") {
    const syntheticWorkout = decision.resource?.workout?.source === "synthetic_judge_fixture";
    $("#approveDecision").textContent = mealApproval ? "Confirm and log" : syntheticWorkout ? "Approve simulation" : "Approve action";
  }
}
...
async function sendCoachMessage(message) {
  ...
  try { const result = await request("/api/coach", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message }) }); pending.remove(); const modeNote = result.mode === "live" ? "" : result.mode === "deterministic_safety" ? `<small class="message-mode">Deterministic safety stop</small>` : `<small class="message-mode">Deterministic demo response</small>`; addMessage("assistant", `${modeNote}<p>${escapeHtml(result.decision.proposal)}</p><button class="inline-action" data-scroll="ledger">See why →</button>`); renderLedger(result.decision); toast(result.mode === "live" ? "Reasoned live with GPT-5.6" : result.mode === "deterministic_safety" ? "Safety stop created and saved" : "Demo trace created and saved"); }
...
$("#analyzeFood").addEventListener("click", async () => {
  ...
  catch (error) { toast(error.message); } finally { button.disabled = false; button.textContent = "Analyze with StrideOS"; }
});
...
async function renderOnboardingReview() {
  $("#onboardingFields").innerHTML = '<div class="review-grid"><article class="review-card review-full"><small>Building analysis</small><strong>Checking your athlete map…</strong><p>No provider session or schedule is being created.</p></article></div>';
  try {
...
function connectorStatusClass(status) {
  ...
  if (["setup_required", "companion_required", "file_or_manual", "manual_only"].includes(status)) return "setup";
  return "planned";
}
...
function renderDataSetup(data) {
  ...
  $("#connectorCount").textContent = `${active} usable routes · ${visibleConnectors.length} sources`;
  $("#connectorGrid").innerHTML = visibleConnectors.map((connector) => { const status = ["available_attended", "attended_session_available"].includes(connector.status) ? "contract_only" : connector.status; return `<article class="connector-card ${connectorStatusClass(status)}">
    <header><span>${escapeHtml(connector.label)}</span><b>${escapeHtml(connectorStatusLabels[status] || humanize(status))}</b></header>
