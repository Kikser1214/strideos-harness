...
3. Install with `codex plugin add strideos@strideos` for Codex CLI or install/enable StrideOS through the ChatGPT desktop Plugins Directory. Restart, open a new Work/Codex task, invoke `@strideos` to start first-time onboarding, and verify it uses grouped rounds while preserving the detailed athlete map. When a provider is selected, verify it confirms provider-specific read scopes, history window, and read timing before retrieval. Then invoke `schedule-coaching` to preview and manually test a morning brief, and ask it to create a Training Circle. Verify the schedule remains explicitly not installed, the test is read-only, the athlete remains the only plan approver, and any unbound identity or persistence is labeled as a demo or local artifact.
...
- **Training Circle:** invoke `build-coach-room`, scope a reviewer to the minimum useful fields, attach a comment to an exact session, and verify the suggestion becomes a separate athlete-visible proposal rather than a silent edit.
